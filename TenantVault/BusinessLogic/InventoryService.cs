    ...
    public class InventoryService(ITenantDataAdapter tenantDataAdapter) : IInventoryService
    {
        ...
        public async Task<Guid> AddVehicleAsync(Vehicle vehicle, CancellationToken cancellationToken)
        {
            ...
            var vehiclesInWarehouse = await CosmosOperationRunner.ExecuteAsync(
                () => _tenantDataAdapter.GetVehiclesByWarehouseAsync(vehicle.TenantId!, vehicle.WarehouseId, cancellationToken));
        ...
        public Task<Vehicle?> GetVehicleAsync(string tenantId, int warehouseId, Guid vehicleId, CancellationToken cancellationToken) =>
            CosmosOperationRunner.ExecuteAsync(() => _tenantDataAdapter.GetVehicleAsync(tenantId, warehouseId, vehicleId, cancellationToken));
        ...
        public Task<IEnumerable<Vehicle>> GetVehiclesByWarehouseAsync(string tenantId, int warehouseId, CancellationToken cancellationToken) =>
            CosmosOperationRunner.ExecuteAsync(() => _tenantDataAdapter.GetVehiclesByWarehouseAsync(tenantId, warehouseId, cancellationToken));

        public Task<IEnumerable<Vehicle>> GetVehiclesByTenantAsync(string tenantId, CancellationToken cancellationToken) =>
            CosmosOperationRunner.ExecuteAsync(() => _tenantDataAdapter.GetVehiclesByTenantAsync(tenantId, cancellationToken));
    }
}
